...
export class ProjectRepository implements IProjectRepository {
    ...
    async findBySlug(slug: string, organization_id: string): Promise<ProjectEntity | null> {
    ...
    async update(id: string, project: ProjectEntity): Promise<ProjectEntity> {
        const toPrisma = ProjectMapper.toPrisma(project);
        const prj = await this.rbacDBService.project.update({ where: { id }, data: toPrisma })
        return ProjectMapper.toDomain(prj)
    };
