...
import { CursorMembersQuery, PaginateMembersQuery } from "@/modules/iam/presentation/dtos/req/member.dto";
import { cursorHelper, paginateHelper, SortableFieldEnum, SortedEnum } from "@/common/pagination";
import { MemberMapper } from "../mappers/member.mapper";
import { Injectable } from "@nestjs/common";

@Injectable()
export class MemberRepository extends CacheRepository implements IMemberRepository {
    ...
    async paginate(pageOptions: PaginateMembersQuery) {
        const { data = [], paginated } =
            await paginateHelper<Prisma.MemberGetPayload<{}>>({
    ...
    async cursorPagination(pageOptions: CursorMembersQuery) {
        const { data = [], paginated } =
            await cursorHelper<Prisma.MemberGetPayload<{}>>({
                query: this.rbacDBService.member,
                pageOptions,
    ...
    async findStaff(staffId: string, projectId: string): Promise<Member | null> {
        const item = await this.getWithCache(`members:staff:${staffId}:project:${projectId}`, async () => {
            return this.rbacDBService.member.findUnique({ where: { staff_id_project_id: { staff_id: staffId, project_id: projectId } } });
        });
        if (!item) return null;
        return MemberMapper.toDomain(item);
    };
    ...
    async findByProjectId(projectId: string): Promise<Member[]> {
        const items = await this.getWithCache(`members:project:${projectId}`, async () => {
            return this.rbacDBService.member.findMany({ where: { project_id: projectId } });
        });
        if (!items) return [];
        return items.map((item) => MemberMapper.toDomain(item));
    };
    ...
    async update(data: any): Promise<Member> {
        const item = await this.rbacDBService.member.update({ where: { id: data.id }, data });
        await this.invalidateCache(`members:${item.id}`);
        return MemberMapper.toDomain(item);
    };
