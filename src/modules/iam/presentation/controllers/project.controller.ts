...
import { CreateProjectDto, UpdateProjectDto } from '@/modules/iam/presentation/dtos/req/project.dto';
import { ProjectResponseDto, PaginateProjectsResponseDto, CursorProjectsResponseDto } from '@/modules/iam/presentation/dtos/res/project-response.dto';
...
export class ProjectController {
    constructor(private readonly projectCmdHandler: ProjectCmdHandler, private readonly prjectQueryHandler: ProjectQueryHandler) { }
    ...
    @ApiResponse({ status: 200, description: 'Project found', type: ProjectResponseDto })
    @ApiResponse({ status: 404, description: 'Project not found' })
    async getBySlug(@Param('slug') slug: string, @Query('organization_id') organization_id: string): Promise<ProjectResponseDto> {
    ...
    @ApiQuery({ name: 'organization_id', required: false, type: String, description: 'Filter by organization' })
    @ApiResponse({ status: 200, description: 'Projects retrieved successfully', type: PaginateProjectsResponseDto })
    ...
    @ApiQuery({ name: 'limit', required: false, type: Number, description: 'Items per page (default: 10, max: 100)' })
    @ApiQuery({ name: 'organization_id', required: false, type: String, description: 'Filter by organization' })
    @ApiResponse({ status: 200, description: 'Projects retrieved successfully', type: CursorProjectsResponseDto })
    async cursorPagination(@Query() query: any): Promise<CursorProjectsResponseDto> {
