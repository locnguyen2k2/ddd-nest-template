...
import { GetOrganizationByIdQuery, GetOrganizationBySlugQuery, ListOrganizationsQuery } from '../../dtos/queries/organization-query.dto';
...
export class OrganizationQueryHandler {
    constructor(@Inject(ORGANIZATION_REPO) private readonly organizationRepository: IOrganizationRepository) { }

    async handleGetOrganizationById(query: GetOrganizationByIdQuery): Promise<Organization | null> {
        return await this.organizationRepository.findById(query.id);
    }

    async handleGetOrganizationBySlug(query: GetOrganizationBySlugQuery): Promise<Organization | null> {
    ...
    async handleListOrganizations(query: ListOrganizationsQuery): Promise<{
        ...
        const paginatedOrganizations = filteredOrganizations.slice(skip, skip + limit);
