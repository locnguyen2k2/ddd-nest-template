...
export class UserCmdHandler {
    constructor(@Inject(USER_REPO) private readonly userRepo: UserRepository,
    ...
    async register(args: RegisterUserArgs): Promise<AuthResponseDto> {
        ...
        const [isUsernameExisted, isPasswordValid, hashedPassword, isEmailExisted] = await Promise.all([
            await this.userService.usernameIsExisted(args.username),
            await this.userService.validatePassword(args.password),
        ...
        switch (true) {
            ...
            case !isPasswordValid:
                throw new BusinessException('400|Password is invalid');
        }
