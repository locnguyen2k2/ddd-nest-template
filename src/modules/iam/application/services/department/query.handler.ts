...
export class DepartmentQueryHandler {
  ...
  async handlePaginate(
    query: PaginateDepartmentsQuery,
  ...
  async handleGetDepartmentById(
    query: GetDepartmentByIdQuery,
  ): Promise<Department | null> {
    return await this.departmentRepository.findById(query.id);
  }

  async handleGetDepartmentBySlug(
    query: GetDepartmentBySlugQuery,
  ): Promise<Department | null> {
    return await this.departmentRepository.findBySlug(
      query.slug,
      query.organization_id,
    );
  }

  async handleGetDepartmentsByOrgId(
    query: GetDepartmentsByOrgIdQuery,
  ): Promise<Department[]> {
