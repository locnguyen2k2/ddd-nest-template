...
export abstract class CacheRepository<T> {
    ...
    constructor(redisConfig: ConfigService<ConfigKeyPaths>, @Inject(CACHE_PORT) private readonly cachePort: CachePort) {
    ...
    protected async invalidateCache(id: string, version?: number | string): Promise<void> {
        const cacheKey = this.buildKey(id, version);
